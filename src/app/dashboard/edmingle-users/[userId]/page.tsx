...
import { ArrowLeft, Calendar, Check, ExternalLink, Info, User, X } from 'lucide-react';
...
const fetchUserDetails = async (userId: string) => {
  try {
    const response = await axios.get(`${env?.BACKEND_URL}/api/edmingle/user/${userId}`);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <CardContent>
            <div className="grid grid-cols-3 gap-4 mb-6">
              <div className="bg-gray-50 rounded-lg p-3 text-center">
                <p className="text-2xl font-bold text-gray-800">{user.enrollments.length}</p>
                <p className="text-xs text-gray-600">Total Courses</p>
              </div>
              <div className="bg-green-50 rounded-lg p-3 text-center">
                <p className="text-2xl font-bold text-green-700">{statusCounts.enrolled || 0}</p>
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <p className="text-sm">
                  {Math.round(((statusCounts.enrolled || 0) / user.enrollments.length) * 100)}%
                </p>
              </div>
              ...
              <div className="flex justify-between items-center">
                ...
                <p className="text-sm">
                  {Math.round(((statusCounts.pending || 0) / user.enrollments.length) * 100)}%
                </p>
              </div>
              <div className="w-full bg-gray-100 rounded-full h-2">
                <div
              ...
              <div className="flex justify-between items-center">
                ...
                <p className="text-sm">
                  {Math.round(((statusCounts.failed || 0) / user.enrollments.length) * 100)}%
                </p>
              </div>
              <div className="w-full bg-gray-100 rounded-full h-2">
                <div
        ...
        <Card>
          <CardHeader className="pb-3">
            ...
            <CardDescription>Most recent enrollment information</CardDescription>
          </CardHeader>
          <CardContent>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="grid grid-cols-2 gap-1 text-sm">
                        ...
                        <p className="font-medium text-gray-700">{latestEnrollment.paymentId}</p>
                      </div>
                    </div>
                  </div>
                );
      ...
      <div className="mt-8">
        <Tabs defaultValue="all" value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="enrolled" className="mt-6">
            <EnrollmentList enrollments={filterByStatus(user.enrollments, 'enrolled')} />
          </TabsContent>

          <TabsContent value="pending" className="mt-6">
            <EnrollmentList enrollments={filterByStatus(user.enrollments, 'pending')} />
          </TabsContent>

          <TabsContent value="failed" className="mt-6">
            <EnrollmentList enrollments={filterByStatus(user.enrollments, 'failed')} />
          </TabsContent>
        </Tabs>
      </div>
    </div>
